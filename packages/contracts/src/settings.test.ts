...
describe("ServerSettings.providerInstances (slice-2 invariant)", () => {
  ...
  it("decodes a fully empty config (legacy on-disk shape) without complaint", () => {
    ...
    expect(decoded.providers.kiro.enabled).toBe(true);
  });
...
describe("provider enabled defaults", () => {
  ...
  it("derives per-driver defaults from the settings schemas", () => {
    ...
    expect(defaultEnabledForDriver(ProviderDriverKind.make("kiro"))).toBe(true);
  ...
  it("resolves instance enabled state with explicit false winning", () => {
    ...
    expect(resolveProviderInstanceEnabled({ driver: kiro, config: {} })).toBe(true);
    ...
    expect(resolveProviderInstanceEnabled({ driver: kiro, enabled: false, config: {} })).toBe(
      false,
    );
    ...
    expect(
      resolveProviderInstanceEnabled({ driver: kiro, enabled: false, config: { enabled: true } }),
    ).toBe(false);
  });
});
