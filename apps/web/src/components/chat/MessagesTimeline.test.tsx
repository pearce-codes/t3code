...
describe("MessagesTimeline", () => {
  ...
  it("replaces a Kiro plan snapshot with its completed state", () => {
    const turnId = TurnId.make("kiro-plan-completion");
    const planActivity = (
      id: string,
      createdAt: string,
      ...
      tone: "info",
      turnId,
      payload: { explanation: "Kiro rendering plan", plan },
    });
    const activities = [
      planActivity("kiro-plan-running", "2026-03-17T19:12:28.000Z", [
        { step: "Inspect event shapes", status: "completed" },
        { step: "Render lifecycle states", status: "inProgress" },
      ]),
      planActivity("kiro-plan-completed", "2026-03-17T19:12:31.000Z", [
        { step: "Inspect event shapes", status: "completed" },
        { step: "Render lifecycle states", status: "completed" },
      ]),
    ];
    const timelineEntries = deriveTimelineEntries([], [], [], deriveTurnPlans(activities));

    const markup = renderToStaticMarkup(
      <MessagesTimeline {...buildProps()} timelineEntries={timelineEntries} />,
    );

    expect(markup).toContain("Render lifecycle states");
    expect(markup).toContain("2/2");
    expect(markup).toContain('aria-expanded="false"');
    expect(markup).not.toContain("1/2");
  });

  it("renders running, failed, and completed Kiro tool lifecycle rows", () => {
    const turnId = TurnId.make("kiro-tool-lifecycle");
    const toolActivity = (
      id: string,
      createdAt: string,
      summary: string,
    ...
    const activities = [
      toolActivity(
        "kiro-tool-running",
        "2026-03-17T19:12:28.000Z",
        "Run Kiro renderer",
        "inProgress",
        "Rendering agent rows",
      ),
      toolActivity(
        "kiro-tool-failed",
        "2026-03-17T19:12:29.000Z",
      ...
      toolActivity(
        "kiro-tool-completed",
        "2026-03-17T19:12:30.000Z",
        "Run focused tests",
        "completed",
        "All Kiro tests passed",
      ),
    ];
    ...
    const runningMarkup = renderToStaticMarkup(
      <MessagesTimeline
        ...
        runningTurnId={turnId}
        timelineEntries={deriveTimelineEntries([], [], [workEntries[0]!])}
      />,
    );
    const failedMarkup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={deriveTimelineEntries([], [], [workEntries[1]!])}
      />,
    );
    const completedMarkup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        timelineEntries={deriveTimelineEntries([], [], [workEntries[2]!])}
      />,
    );

    expect(runningMarkup).toContain("Running Rendering");
    expect(runningMarkup).toContain('aria-expanded="false"');
    expect(failedMarkup).toContain("Worked for");
    expect(failedMarkup).toContain('aria-expanded="false"');
    expect(completedMarkup).toContain("Worked for");
