...
export const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription(
    "Explicit Pearce Codes data directory; runtime state is stored under userdata (equivalent to PEARCE_CODES_HOME).",
  ),
  Flag.optional,
);
...
const EnvServerConfig = Config.all({
  ...
  pearceCodesHome: Config.string("PEARCE_CODES_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
...
export const resolveServerConfig = (
  ...
  Effect.gen(function* () {
    ...
    const explicitBaseDir = resolveOptionPrecedence(
      ...
      Option.fromUndefinedOr(env.pearceCodesHome),
