...
describe("ssh tunnel scripts", () => {
  it("builds the remote t3 runner with npx and npm fallbacks", () => {
    ...
    assert.include(
      script,
      "require_installed_t3_cli npx --yes --package '@pearcecodes/t3code@latest'",
    );
    assert.include(
      script,
      "require_installed_t3_cli npm exec --yes --package '@pearcecodes/t3code@latest'",
    );
    assert.include(script, "npm produced no pearcecodes executable");
