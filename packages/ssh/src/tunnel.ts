...
export const DEFAULT_REMOTE_PORT = 5656;
...
# the npx cache without a CLI executable. \`npx --yes\` then exits 0 without
...
require_installed_t3_cli() {
  T3_CLI_PATH="$("$@" -- sh -c 'command -v @@T3_CLI_BIN@@' || true)"
  if [ -n "$T3_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no @@T3_CLI_BIN@@ executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
}
