...
export function UsagePage() {
  ...
  const timeValueColumnWidth = `${60 / (activeProviders.length + 3)}%`;
    ...
    <div className="flex w-full min-w-0 items-center gap-3">
      ...
      <div className="ms-auto hidden min-w-0 items-center justify-end gap-2 lg:flex">
        <ToggleGroup
          ...
          onValueChange={(next) => {
            const value = next[0];
            if (value === "cost" || value === "tokens" || value === "credits") setMetric(value);
          }}
        >
          {(["cost", "tokens", "credits"] as const).map((option) => (
            <Toggle key={option} value={option}>
              {option === "cost" ? "Cost" : option === "credits" ? "Credits" : "Tokens"}
            </Toggle>
          ))}
        </ToggleGroup>
        <ToggleGroup
      ...
      <div className="ms-auto flex min-w-0 items-center justify-end gap-1 lg:hidden">
        ...
        >
          ...
          >
            <SelectValue>
              {metric === "cost" ? "Cost" : metric === "credits" ? "Credits" : "Tokens"}
            </SelectValue>
          </SelectTrigger>
                        ...
                        <div key={provider} className="flex flex-col gap-1">
                          <div className="flex items-baseline justify-between gap-4">
                            ...
                            <span className="shrink-0 text-sm font-medium text-foreground tabular-nums">
                              {metric === "cost"
                                ? `${provider === "kiro" ? "~" : ""}${formatUsd(totals?.costUsd ?? 0)}`
                                : metric === "credits"
                                  ? formatCredits(totals?.credits ?? 0)
                    ...
                    <table className="w-full table-fixed text-sm">
                      ...
                      <tbody>
                        {breakdownPeriods.length === 0 ? (
                          <tr>
                            <td
                              colSpan={activeProviders.length + 4}
