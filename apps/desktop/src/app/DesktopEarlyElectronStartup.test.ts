...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { PEARCE_CODES_HOME: "/home/user/.pearce-codes-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.pearce-codes-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        PEARCE_CODES_HOME: "/home/user/.pearce-codes-test",
    ...
    assert.deepEqual(options, {
      linuxWmClass: "pearce-codes-dev",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.pearce-codes/dev when PEARCE_CODES_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.pearce-codes/dev/desktop-settings.json");
  ...
  it("treats whitespace-only PEARCE_CODES_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        PEARCE_CODES_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
